    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="center" label="操作" min-width="200" fixed="right">
          <template #default="scope">
            <el-button
              v-if="scope.row.claim.auditStatus!==2 && scope.row.claim.auditStatus!==3"
              type="success"
              icon="select"
              size="small"
              @click="approveClaimFunc(scope.row)"
            >通过
            </el-button>
            <el-button
              v-if="scope.row.claim.auditStatus!==2 && scope.row.claim.auditStatus!==3"
              ...
              icon="closeBold"
              size="small"
              @click="rejectClaimFunc(scope.row)"
            >拒绝
            </el-button>
            <el-tag
              v-if="scope.row.claim.auditStatus===2"
              type="success"
              effect="dark"
              size="large"
            >已审批通过
            ...
            <el-tag
              v-if="scope.row.claim.auditStatus===3"
              type="danger"
              effect="dark"
              size="large"
            >已审批拒绝
...
const tableData = ref([])
const searchInfo = ref({ auditClaim: true })
const templateData = ref([])

// 重置
const onReset = () => {
  searchInfo.value = { auditClaim: true }
  getTableData()
}
...
const approveClaimFunc = async(apply) => {
  ElMessageBox.confirm('确定要通过吗?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async() => {
    const res = await approveClaim(apply)
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '提交成功'
...
const rejectClaimFunc = async(apply) => {
  ElMessageBox.confirm('确定要拒绝吗?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async() => {
    const res = await rejectClaim(apply)
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '提交成功'
